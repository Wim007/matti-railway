...
import { CheckCircle2, Circle, XCircle, TrendingUp, Target } from "lucide-react";
...
export default function Actions() {
  ...
  const handleCardClick = (action: typeof actions[0]) => {
    if (action.goalId) {
      // Goal-actie → navigeer naar Goal Detail pagina
      navigate(`/goals/${action.goalId}`);
    } else if (action.conversationId) {
      // Losse actie → navigeer naar bijbehorende chat
      navigate(`/chat?conversationId=${action.conversationId}`);
    }
  };
        ...
        <div className="px-4 space-y-3">
          {actions.length === 0 ? (
            ...
            actions.map((action) => {
              ...
              const isGoalAction = !!action.goalId;
              const isClickable = isGoalAction || !!action.conversationId;

              return (
                <div
                  key={action.id}
                  className={`bg-card rounded-2xl shadow-sm p-4 border border-border border-l-4 transition-all ${
                    isClickable ? "cursor-pointer hover:shadow-md active:scale-[0.99]" : ""
                  }`}
                  style={{ borderLeftColor: theme.colors.gradient[0] }}
                  onClick={isClickable ? () => handleCardClick(action) : undefined}
                >
                  {/* Theme Badge + Goal label */}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {/* Buttons only for non-goal pending actions */}
                    {isPending && !isGoalAction && (
                    ...
                    {/* Goal action: toon "Bekijk doel" pijl */}
                    {isPending && isGoalAction && (
